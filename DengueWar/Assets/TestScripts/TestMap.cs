...
public class TestMap : MonoBehaviour
{
	...
	Gps gps = new Gps (); //<--------------------------------------------------------------------------------------GPS pra pegar a posicao longitude e latidute


	bool Toolbar(Map map)
	{
		GUI.matrix = Matrix4x4.Scale(new Vector3(guiXScale, guiXScale, 0.02f));
       ...
       // if (GUILayout.RepeatButton("+", GUILayout.ExpandHeight(true)))
        ...
        //if (GUILayout.Button("Visao", GUILayout.ExpandHeight(true)))
		//{
        ...
        if (GUILayout.Button("Centro", GUILayout.ExpandHeight(true)))
        {
            map.CenterOnLocation();
        }
		if (GUILayout.Button("MARCAR", GUILayout.ExpandHeight(true)))
		{
			//<--------------------------------------------------------------------------------------------------------------CLICOU AQUI TEM MARCAR
	...
	Start()
	{
		...
		map.CenterWGS84 = new double[2] { gps.getLo () , gps.getLa () }; //<---------------------------------------------------------------------------------AQUI A POSICAO DO MAPA
		...
		map.CreateMarker<Marker>("test marker - 9 rue Gentil, Lyon", new double[2] { -35.8833 , -7.2167 }, markerGO);

		markerGO = Instantiate(go) as GameObject;
		map.CreateMarker<Marker>("test marker - 31 rue de la Bourse, Lyon", new double[2] { -35.906374488449046 , -7.2508454795846085 }, markerGO);
		
		markerGO = Instantiate(go) as GameObject;
		map.CreateMarker<Marker>("test marker - 1 place St Nizier, Lyon", new double[2] { -35.91113540945048, -7.246239681127994 }, markerGO);
